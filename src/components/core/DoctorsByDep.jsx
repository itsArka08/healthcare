                ...
                {data?.map((item, index) => (
                  <div key={index} class="col-md-4">
                    <div class="card mb-3 shadow-sm border-0" style={{ maxWidth: '300px' }}>
                      <img src="https://media.istockphoto.com/id/1316501954/vector/female-doctor-with-black-hair-standing-hands-in-pocket.jpg?s=612x612&w=0&k=20&c=sTV-oLBqna2ZuEG1qNwZl4LgHj4WtFkrN8bDnHQPBxo=" alt="" class="card-img-top" style={{ height: '250px', objectFit: 'cover' }} />
                      <div class="card-body">
                        <h5 class="card-title">{item?.name}</h5>
                        <p class="card-text">{item?.description.slice(0, 100)}</p>
                        <Link to={`/doctor-single/${item?._id}`} class="btn btn-main">Make Appointment</Link>
                      </div>
                    </div>
                  </div>
                ))}
