...
import { GetAllChildCare } from "../../Redux/ChildCareSlice";
...
const ChildCare = () => {
  ...
  const { all_child_care_data } = useSelector((state) => state?.AllChildCare);

  const dname = all_child_care_data?.data?.[0]?.departmentName
  const doctor = all_child_care_data?.data?.[0]?.doctor_id

  useEffect(() => {
    dispatch(GetAllChildCare());
  }, [dispatch]);
